# {{ slot_label }} — Memory-Safety & Exploitation
...
- Read disassembly/decompilation (Ghidra/`gdb`) to locate a vulnerable function.
- Understand modern mitigations (ASLR, stack canaries, NX/DEP) **and the global shift to memory-safe languages** (Rust/Go) — see the *Secure by Design* / CISA & ONCD memory-safety roadmaps.
...
2. Overwrite the return address to reach a `win()` function (or use the format-string bug to leak/overwrite memory).
